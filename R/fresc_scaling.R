...
#' @param spp_pa Presence absence data for each species
#' @param all_loc Vector containing all unique locations in the occupany data
...
#'
#' @return A list comprised of two elements. The first element `locs` contains a
#'   `data.frame` with the location metrics while the the second `freq` contains
#'   a `data.frame` with the re-scaled species frequencies. These two outputs
#'   are equivalent to `samples.txt` and `frequencies.out` output files produced
#'   by the original frescalo fortran program.
#'
...
#'   \item{freq}{the localisied frequency for that species & neighbourhood before rescaling}
#'   \item{freq_1}{the local frequency for that species & neighbourhood after rescaling}
...
#'   species for the local neighbourhood (`1` = benchmark
#'   species/taxa)}
#'
#' @export
#'
#' @description Determines the frequency scaling factors required in order to
#'   attempt to correct for variation in recording effort.
#'
#'
#' @examples
#'
